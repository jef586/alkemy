...
describe "POST /admin/posts", type: :request do
  let(:endpoint) { post admin_posts_path, params: correct_params, as: :json }
  let(:correct_params) { {
  ...
  context "when user is an admin" do
    ...
    context "and the parameters are correct" do
      ...
      it "saves ONE post" do
        expect { endpoint }.to change(Post, :count)
      end
      it "succesfully creates a post with the correct name" do
        endpoint
        expect(data["name"]).to eq(correct_params[:name])
      end
      it "succesfully creates a post with the correct content" do
        endpoint
    ...
    context "but some parameter is invalid or missing" do
      before { post admin_posts_path, params: { name: correct_params[:name] }, as: :json }
