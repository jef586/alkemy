...
describe "PUT /admin/testimonials/:id", type: :request do
  let(:endpoint) { put admin_testimonial_path(testimonial), params: correct_params, as: :json }
  let(:correct_params) { { name: "Updated testimonial name", content: "Updated testimonial content" } }
  ...
  context "when user is an admin" do
    ...
    context "but the testimonial doesn't exist" do
      before { put admin_testimonial_path(200), params: correct_params, as: :json }

      it "returns an HTTP status 404" do
        expect(response).to have_http_status(404)
      end
      it "returns error message: 'Couldn't find testimonial...'" do
        expect(body_json["error"]).to match(/Couldn't find Testimonial/)
      end
    end
  end
