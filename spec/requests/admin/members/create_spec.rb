...
describe "POST /admin/members", type: :request do
  let(:endpoint) { post admin_members_path, params: correct_params, as: :json }
  let(:correct_params) { {
    name: "Pepe"
    } }
  ...
  context "when user is an admin" do
    ...
    context "and the parameters are correct" do
      ...
      it "succesfully creates a member with the correct name" do
        endpoint
        expect(data["name"]).to eq(correct_params[:name])
      end
      it "succesfully creates a member with the correct content" do
        endpoint
    ...
    context "but some parameter is invalid or missing" do
      before { post admin_members_path, params: { last_name: correct_params[:name] }, as: :json }
